...
func (h *ChatHandler) ChatHandle(c *gin.Context) {
	...
	go func() {
		for {
			...
			fmt.Println("这是接收到的回复消息 ", utils.InterfaceToString(message.Content))
			// 回复消息
...
func (h *ChatHandler) Tokens(c *gin.Context) {
	...
	if data.Text == "" && data.ChatId != "" {
		...
		fmt.Println("where error ", res.Error.Error())
		// && res.Error.Error() != "record not found"
		if res.Error != nil {
			resp.ERROR(c, res.Error.Error())
			return
		}
		// temp := userId.(float64)
		// item.UserId = uint(temp)
	...
	if err != nil {
		fmt.Println("Caltokens error ", err.Error())
...
func (h *ChatHandler) doRequest(ctx context.Context, req types.ApiRequest, session *types.ChatSession, apiKey *model.ApiKey) (*http.Response, error) {
	...
	var apiURL string
	fmt.Println("这是 apikey ", apiKey)
	fmt.Println("这是 apiUrl ", apiURL)
	switch session.Model.Platform {
	case types.Azure:
	...
	fmt.Println("http请求对象的requestBody", string(requestBody))
	if err != nil {
		return nil, err
	}
