...
func (h *ChatHandler) sendOpenAiMessage(
	...
	fmt.Println("这里是openai send")
	// if len(req.Messages)==1{

	// }
	fmt.Println("这里是req的tools哟")
	fmt.Println(req.Tools)
	fmt.Println()
	...
	fmt.Println("先打印Header")
	for k, v := range response.Header {
		fmt.Println(k, v)
	}
	// fmt.Println("现在要打印response 内容了")
	// body, err := io.ReadAll(response.Body)
	// if err != nil {
	// fmt.Println("读取Body ", body)
	// }
	...
	if strings.Contains(contentType, "text/event-stream") {
		...
		var isNew = true
		fmt.Println("我进来event-stream了吗")
		for scanner.Scan() {

			// fmt.Println("我进入Scan了吗")
			line := scanner.Text()
			fmt.Println("line ", line)
			if !strings.Contains(line, "data:") || len(line) < 30 {
				// fmt.Println("scann 的第一个 continue")
				// fmt.Println(line)
			...
			fmt.Println("break point 0")
			// if err != nil || len(responseBody.Choices) == 0 { // 数据解析出错
			if err != nil { // 数据解析出错
				fmt.Println(err)
				fmt.Println("这是解析后的", responseBody)
			...
			if !utils.IsEmptyValue(tool) {
				...
				if res.Error == nil {
					...
					fmt.Println("utils.IsE error ", res.Error.Error())
				}
				continue
			}
		...
		if err := scanner.Err(); err != nil {
			fmt.Println("scanner err ", scanner.Err().Error())
			if strings.Contains(err.Error(), "context canceled") {
				logger.Info("用户取消了请求：", prompt)
			} else {
		...
		fmt.Println("这是要发送的消息 ", contents)
		// 消息发送成功
		if len(contents) > 0 {
			if message.Role == "" {
				message.Role = "assistant"
			}
			message.Content = strings.Join(contents, "") //添加回复消息
			...
			fmt.Println("当前会话信息", chatItem)
			res = h.DB.Where("chat_id = ?", session.ChatId).First(&chatItem)
			if res.Error != nil {
				fmt.Println("res error in chatitem ", res.Error.Error())
		...
		body, err := io.ReadAll(response.Body)
		fmt.Println("openai 回复", string(body))
		if err != nil {
